                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            <td>
                                                ...
                                                <br>
                                                <strong class="text-danger">{{ __('main.available') }}: {{
                                                    $cartItem->availableQuantity }}</strong>
                                                @endif
                                            </td>
                                            <td class="text-nowrap text-right"> {{ Helper::formatPrice($cartItem->getPriceSum()) }}
                                            </td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                            ...
                            <div>
                                ...
                                <div class="form-group">
                                    ...
                                    <input type="text" name="address"
                                        ...
                                        value="{{ old('address', optional($address)->address_line_1) ?? '' }}">
                                    @error('address')
                                    <span class="invalid-feedback" role="alert">
                                        <strong>{{ $message }}</strong>
                                    </span>
                                ...
                                <div class="form-group">
                                    ...
                                    @php
                                    $checkedPaymentMethodKey = old('payment_method_id') ?: 1; // default = 1 - cash
                                    @endphp
                                    @foreach(\App\Order::paymentMethods() as $paymentMethodKey => $paymentMethod)
                                    @if ($paymentMethodKey == \App\Order::PAYMENT_METHOD_ZOODPAY_INSTALLMENTS && ($cart->getTotal() > config('services.zoodpay.max_limit') || $cart->getTotal() < config('services.zoodpay.min_limit')) )
                                        @continue
                                    @endif

                                    <div class="form-check d-inline-block mr-2">
                                        <input class="form-check-input"
                                            id="payment_method_{{ $paymentMethodKey }}" type="radio"
                                            name="payment_method_id" value="{{ $paymentMethodKey }}"
                                            @if($checkedPaymentMethodKey==$paymentMethodKey) checked @endif
                                            required>
                                        <label class="form-check-label"
                                            for="payment_method_{{ $paymentMethodKey }}">
                                            {{ $paymentMethod }}
                                            {{-- <div>
                                                @if ($paymentMethodKey == \App\Order::PAYMENT_METHOD_ZOODPAY_INSTALLMENTS)
                                                @php
                                                $description = Helper::staticText('zoodpay_payment_description');
                                                $zoodpayInstallments = (int)config('services.zoodpay.installments');
                                                if ($zoodpayInstallments < 1) { $zoodpayInstallments=1; } @endphp <span
                                                    class="text-gray">
                                                    {{ $description->description }}.
                                                    {{ __('main.price_per_month', ['price' => Helper::formatPrice($cart->getTotal()
                                                    / $zoodpayInstallments)]) }}.
                                                    <a href="#zoodpay-terms-modal" data-toggle="modal">{{
                                                        __('main.terms_of_service') }} <i class="fas fa-external-link-alt"></i></a>
                                                    <img src="{{ asset('images/payment/zoodpay/zpi-' . app()->getLocale() . '.jpg') }}"
                                                        alt="Zoodpay" class="img-fluid ml-2" style="max-width: 70px">
                                                    </span>
                                                    @else
                                                    <span class="text-gray">{{
                                                        \App\Order::paymentMethodsDescriptions()[$paymentMethodKey] }}</span>
                                                    @endif
                                                </div> --}}
                                        </label>
                                    </div>
        ...
        $('.get-location-btn').on('click', function(e){
            ...
            function getLocation() {
                ...
                if (navigator.geolocation) {
                    let getOptions = {maximumAge: 10000, timeout: 5000, enableHighAccuracy: true};
