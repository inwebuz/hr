    ...
    public function products(Request $request)
    {
        ...
        foreach ($reader->getSheetIterator() as $sheet) {
            foreach ($sheet->getRowIterator() as $key => $row) {
                ...
                $productOptionAttributeIDs = [];
                $productOptionAttributeValues = [];
                $productOptionAttributeValueIDs = [];
                ...
                foreach($otherAttributesRaw as $value) {
                    ...
                    $otherAttributesArray[Str::limit(trim($value[0]), 191, '')] = Str::limit(trim($value[1]), 191, '');
                }
                ...
                foreach($productOptionAttributeIDs as $productOptionAttributeID) {
                    $productAllAttributes[$productOptionAttributeID] = ['used_for_variations' => 1];
                }
                foreach($productOtherAttributeIDs as $productOtherAttributeID) {
                    $productAllAttributes[] = $productOtherAttributeID;
                }
                foreach($productOptionAttributeValueIDs as $productOptionAttributeValueID) {
                    $productAllAttributeValues[] = $productOptionAttributeValueID;
                }
